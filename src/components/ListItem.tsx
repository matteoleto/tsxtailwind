...
function ListItem(prop: Messaggio): ReactElement<HTMLElement> {
  ...
  if (prop.sender) {
    ...
    chId = `flex items-center justify-between p-2 bg-${
      color
    }-200 rounded-md mb-2`; // Assign appropriate class or ID for receiver
  }
        ...
        onClick={() => {
          const textareaObj = document
            .getElementsByClassName(
              `flex-grow bg-${
                color
              }-500 text-white rounded-full p-2 focus:outline-none resize-none ml-4 bg-blue-500`,
            )
            .item(0);
